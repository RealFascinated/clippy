...
const AuthForm = ({
  ...
  if (!title) {
    title =
      (type === "auto" ? "Welcome" : type === "login" ? "Login" : "Register") +
      ` to ${env.NEXT_PUBLIC_WEBSITE_NAME}`;
  }
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        {authOptions.socialProviders && (
          <>
            <OAuthProviders authOptions={authOptions} setError={setError} />
      ...
      {!isProduction() && (
        ...
        >
          Operating in{" "}
          <span className="text-[#DA8702] font-medium">Development</span>
        </div>
      )}
    </div>
  );
...
const LegalFooter = ({
  ...
  <footer className="flex flex-col gap-1 items-center text-sm text-muted-foreground">
    ...
    <div className="flex gap-3.5 items-center">
      {termsAndConditions && (
        <LegalLink href={termsAndConditions}>Terms of Service</LegalLink>
      )}
      {privacyPolicy && (
  ...
  return (
    <GenericFormView submitText="Register" onSubmit={handleRegistration}>
      ...
      <FormInput
        ...
        icon={Mail}
        required
        defaultValue={prefilledData?.isEmail ? prefilledData?.input : undefined}
        disabled={prefilledData?.isEmail && !!prefilledData?.input}
      />
  ...
  const handleLogin = async (form: FormData) => {
    ...
    if (!promptPassword) {
      const doesUserExist: boolean =
        (
          await request.post<UserExistsResponse>("/api/auth/user-exists", {
            data: {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <FormInput
          className={cn(
            promptPassword && "opacity-50 cursor-not-allowed bg-muted"
          )}
...
const OAuthProviders = ({
  ...
  return (
    <div className="flex flex-wrap gap-1.5 justify-center items-center">
      {Object.values(authOptions.socialProviders ?? {}).map((provider: any) => (
        <OAuthProvider
          key={provider.id}
          provider={provider}
          showNames={showNames}
...
const FormInput = ({
  ...
  <div className="flex flex-col gap-2.5">
    ...
    <div className="relative">
      ...
      <Input className={cn(Icon && "pl-8", className)} name={name} {...props} />
    </div>
  </div>
);
